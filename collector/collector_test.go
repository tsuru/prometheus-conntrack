...
func TestCollector(t *testing.T) {
	...
	assert.Contains(t, lines, `conntrack_workload_connections{container="my-container1",destination="192.168.50.4:2375",destination_name="alice-service",direction="outcomming",label_app="app1",protocol="tcp",state="ESTABLISHED"} 2`)
	assert.Contains(t, lines, `conntrack_workload_connections{container="my-container1",destination="192.168.50.5:2376",destination_name="bob-service",direction="outcomming",label_app="app1",protocol="tcp",state="ESTABLISHED"} 1`)
	assert.Contains(t, lines, `conntrack_workload_connections{container="my-container1",destination=":7070",destination_name="",direction="incomming",label_app="app1",protocol="tcp",state="ESTABLISHED"} 1`)
	...
	assert.Equal(t, http.StatusOK, rr.Code)
	lines = strings.Split(rr.Body.String(), "\n")
	assert.Contains(t, lines, `conntrack_workload_connections{container="my-container1",destination="192.168.50.4:2375",destination_name="alice-service",direction="outcomming",label_app="app1",protocol="tcp",state="ESTABLISHED"} 1`)
	assert.Contains(t, lines, `conntrack_workload_connections{container="my-container1",destination="192.168.50.5:2376",destination_name="bob-service",direction="outcomming",label_app="app1",protocol="tcp",state="ESTABLISHED"} 0`)
}

func TestPerformMetricClean(t *testing.T) {
	collector := &ConntrackCollector{}
	now := time.Now().UTC()
	collector.lastUsedTuples.Store(accumulatorKey{workload: "w1", state: "estab", protocol: "tcp", destination: destination{ip: "blah"}}, now.Add(time.Minute*-60))
	collector.lastUsedTuples.Store(accumulatorKey{workload: "w2", state: "estab", protocol: "tcp", destination: destination{ip: "blah"}}, now)
	collector.lastUsedTuples.Store(accumulatorKey{workload: "w3", state: "estab", protocol: "tcp", destination: destination{ip: "blah"}}, now.Add(time.Minute*60))
